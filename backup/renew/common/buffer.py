...
class Buffer():
	...
	def _prepare_batch(self, td):
		...
		最终返回的形状全部是 Batch-first :
			obs     (B, H+1, D)
			action  (B, H,   A)
			reward  (B, H,   1)
			task    (B,) / (B,task_dim) / None
		"""
		td = td.select("obs", "action", "reward", "task", strict=False) \
			.to(self._device, non_blocking=True)

		# -------------- 统一到 batch-first -----------------
		obs    = td["obs"].permute(1, 0, 2).contiguous()          # (B,H+1,D)
		action = td["action"][1:].permute(1, 0, 2).contiguous()   # (B,H,A)
		reward = td["reward"][1:].unsqueeze(-1).permute(1, 0, 2)  # (B,H,1)

		task = td.get("task", None)
		if task is not None:                                      # keep (B,) or (B,d)
