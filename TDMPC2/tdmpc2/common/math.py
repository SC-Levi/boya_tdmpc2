...
def two_hot(x, cfg):
	...
	soft_two_hot = torch.zeros(x_processed.shape[0], cfg.num_bins, device=x.device, dtype=x.dtype)
	
	# Convert to long indices
	bin_idx = bin_idx.long()
	
	# Use non-in-place scatter operations
...
def two_hot_inv(x, cfg):
	...
	# Avoid in-place operations by creating a new tensor with softmax
