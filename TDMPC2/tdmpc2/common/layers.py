...
class Ensemble(nn.Module):
	...
	def __iter__(self):
		"""Return an iterator over the modules to make Ensemble iterable"""
		# Create dummy modules that simulate the original modules
		modules = []
		for i in range(len(self)):
			modules.append(lambda x, i=i, self=self: self._call(self.params[i], x))
		return iter(modules)
		
	def __getitem__(self, idx):
		"""Access a specific model by index"""
		if not isinstance(idx, int) or idx < 0 or idx >= len(self):
			raise IndexError(f"Index {idx} out of range (0, {len(self)})")
		return lambda x, idx=idx, self=self: self._call(self.params[idx], x)
