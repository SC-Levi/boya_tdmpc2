...
class TDMPC2(torch.nn.Module):
	...
	def _estimate_value(self, z, actions, task):
		...
		# 1) rollout latent sequence in a single call (batch-first)
		a_bt          = actions.permute(1, 0, 2).contiguous()          # [B,T,A]
		z_seq, _      = self.model.dynamics_vectorized(z, a_bt)        # [B,T,D]

		# 2) flatten time & batch → [B*T, …]
		z_flat        = z_seq.reshape(B * T, D)                        # [B*T,D]
		a_flat        = a_bt .reshape(B * T, A)                        # [B*T,A]

		# 3) reward distribution & Q
		reward_dist,_ = self.model.reward(z_flat, a_flat, task=task)   # [B*T,R] or [B*T,1]
		q_flat        = self.model.Q     (z_flat, a_flat, task=task,
								   return_type='mean')          # [B*T,1]

		# 4) convert two-hot → scalar if需要
		if reward_dist.dim() == 2 and reward_dist.size(-1) > 1:
			r_flat = math.two_hot_inv(reward_dist, self.cfg).unsqueeze(-1)  # [B*T,1]
		else:
			r_flat = reward_dist                                           # [B*T,1]

		# 5) reshape back & 折扣累积
		r = r_flat.view(B, T)
		q = q_flat.view(B, T)
		v_hat = (r + self.cfg.gamma * q).mean(dim=1, keepdim=True)     # [B,1]
		return v_hat
	...
	def _plan(self, obs, t0=False, eval_mode=False, task=None):
		...
		for _ in range(self.cfg.iterations):
			...
			elite_value  = value[elite_idxs]                       # [E]
			elite_actions= actions[:, elite_idxs]                  # [H,E,A]
