...
from moore.utils.timer import timed, sync_gpu
import time
...
class MTSAC(SAC):
    ...
    def fit(self, dataset, **info):
        ...
        sync_gpu(); print(f"[TIME] replay_sampling: {(time.perf_counter()-t0)*1000:7.2f} ms")
        ...
        if fit_condition:
            ...
            if self._replay_memory[0].size > self._warmup_transitions(): #first or any
                ...
                sync_gpu(); print(f"[TIME] actor_update: {(time.perf_counter()-t0)*1000:7.2f} ms")
            ...
            sync_gpu(); print(f"[TIME] critic_update: {(time.perf_counter()-t0)*1000:7.2f} ms")
