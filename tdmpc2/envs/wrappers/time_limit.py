...
class TimeLimit(gym.Wrapper):
    ...
    def step(self, action):
        ...
        Returns:
            The environment step ``(observation, reward, done, truncated, info)`` with "TimeLimit.truncated"=True
        ...
        if len(ret) == 5:
            observation, reward, done, truncated, info = ret
        else:                                   # inner env is old-API (4 values)
